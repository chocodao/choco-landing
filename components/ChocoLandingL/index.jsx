import React from "react";
import Component81 from "../Component81";
import Component91 from "../Component91";
import Component92 from "../Component92";
import Component41 from "../Component41";
import Component111 from "../Component111";
import Component101 from "../Component101";
import X622369b64625dd72cd6e31a12 from "../X622369b64625dd72cd6e31a12";
import X622369b7e403e7ddc4ce09396 from "../X622369b7e403e7ddc4ce09396";
import Component151 from "../Component151";
import Component152 from "../Component152";
import Component171 from "../Component171";
import Component172 from "../Component172";
import Component181 from "../Component181";
import Component18122 from "../Component18122";
import Component191 from "../Component191";
import Component1912 from "../Component1912";
import "./ChocoLandingL.css";

class ChocoLandingL extends React.Component {
  render() {
    const {
      overlapGroup6,
      chocolateBar_1F36B1,
      choco1,
      finance1,
      chocoIsBuildingBa,
      bar21,
      howToParticipate,
      bar11,
      bar12,
      fallingchoco,
      thetaSNativeDecen,
      overlapGroup11,
      chocolateBar_1F36B2,
      path3,
      path6,
      choco2,
      finance2,
      path5,
      products,
      learn,
      contactUs,
      bar22,
      bar13,
      component81Props,
      component91Props,
      component921Props,
      component922Props,
      component411Props,
      component412Props,
      component413Props,
      component111Props,
      component101Props,
      component1511Props,
      component1512Props,
      component152Props,
      component1513Props,
      component171Props,
      component181Props,
      component18122Props,
      component191Props,
      component1912Props,
    } = this.props;

    return (
      <div className="container-center-horizontal">
        <div className="choco-landing-l screen">
          <div className="flex-row-11">
            <div className="overlap-group-34" style={{ backgroundImage: `url(${overlapGroup6})` }}>
              <img className="chocolate-bar_1f36b-23" src={chocolateBar_1F36B1} />
            </div>
            <div className="overlap-group-35">
              <div className="choco-23 montserrat-black-white-32px">{choco1}</div>
              <div className="finance-24 montserrat-black-white-14px">{finance1}</div>
            </div>
            <Component81 className={component81Props.className} />
            <Component91 className={component91Props.className} />
            <Component92 governance={component921Props.governance} className={component921Props.className} />
            <Component92 governance={component922Props.governance} className={component922Props.className} />
            <Component41 className={component411Props.className}>{component411Props.children}</Component41>
          </div>
          <div className="overlap-group-container-11">
            <div className="overlap-group15">
              <div className="overlap-group3-2">
                <div className="flex-col-24">
                  <div className="choco-is-building-ba-12 montserrat-normal-white-20px">{chocoIsBuildingBa}</div>
                  <div className="component-container-58">
                    <Component41 className={component412Props.className}>{component412Props.children}</Component41>
                    <Component41 className={component413Props.className}>{component413Props.children}</Component41>
                  </div>
                  <img className="bar2-24" src={bar21} />
                </div>
                <div className="overlap-group-container-12">
                  <div className="overlap-group-36">
                    <div className="how-to-participate-12 montserrat-semi-bold-white-70px">{howToParticipate}</div>
                    <div className="overlap-group8-13">
                      <Component111
                        meltingAllowsYouT={component111Props.meltingAllowsYouT}
                        className={component111Props.className}
                        component42Props={component111Props.component42Props}
                      />
                      <img className="bar1-36" src={bar11} />
                    </div>
                  </div>
                  <div className="overlap-group7-12">
                    <Component101
                      fire={component101Props.fire}
                      moldersPlayAnImpo={component101Props.moldersPlayAnImpo}
                      className={component101Props.className}
                      component42Props={component101Props.component42Props}
                    />
                    <img className="bar1-37" src={bar12} />
                  </div>
                </div>
              </div>
              <img className="fallingchoco-7" src={fallingchoco} />
              <div className="thetas-nativedecen-12 montserrat-semi-bold-white-70px">{thetaSNativeDecen}</div>
            </div>
            <div className="overlap-group5">
              <div className="flex-row-12">
                <div className="flex-col-25">
                  <div className="overlap-group-34" style={{ backgroundImage: `url(${overlapGroup11})` }}>
                    <img className="chocolate-bar_1f36b-23" src={chocolateBar_1F36B2} />
                  </div>
                  <a href="https://twitter.com/chocofinance">
                    <div className="x622369b6a395099f1a779d22_1-12">
                      <img className="path-3-6" src={path3} />
                    </div>
                  </a>
                  <a href="https://t.me/chocofinance">
                    <div className="x622369b6bf2cca49909d6eb5_4-12">
                      <img className="path-6-5" src={path6} />
                    </div>
                  </a>
                </div>
                <div className="flex-col-26">
                  <div className="overlap-group1-13">
                    <div className="choco-23 montserrat-black-white-32px">{choco2}</div>
                    <div className="finance-25 montserrat-black-white-14px">{finance2}</div>
                  </div>
                  <div className="b-container-4">
                    <X622369b64625dd72cd6e31a12 />
                    <a href="https://github.com/chocodao">
                      <div className="x622369b636f18be6eacf49c4_3-8">
                        <img className="path-5-11" src={path5} />
                      </div>
                    </a>
                  </div>
                  <X622369b7e403e7ddc4ce09396 />
                </div>
                <div className="flex-col-27">
                  <div className="products-12 montserrat-normal-white-30px">{products}</div>
                  <div className="component-container-59">
                    <Component151>{component1511Props.children}</Component151>
                    <Component151 className={component1512Props.className}>{component1512Props.children}</Component151>
                  </div>
                  <div className="component-container-60">
                    <Component152>{component152Props.children}</Component152>
                    <Component151 className={component1513Props.className}>{component1513Props.children}</Component151>
                  </div>
                </div>
                <div className="flex-col-28">
                  <div className="learn-24 montserrat-normal-white-30px">{learn}</div>
                  <div className="component-container-61">
                    <Component171>{component171Props.children}</Component171>
                    <Component172 />
                  </div>
                  <div className="component-container-62">
                    <Component181>{component181Props.children}</Component181>
                    <Component18122>{component18122Props.children}</Component18122>
                  </div>
                </div>
              </div>
              <div className="flex-col-29">
                <div className="flex-row-13">
                  <div className="contact-us-9 montserrat-normal-white-30px">{contactUs}</div>
                  <img className="bar2-25" src={bar22} />
                </div>
                <div className="component-container-63">
                  <Component191>{component191Props.children}</Component191>
                  <Component1912>{component1912Props.children}</Component1912>
                </div>
              </div>
              <img className="bar1-38" src={bar13} />
            </div>
          </div>
        </div>
      </div>
    );
  }
}

export default ChocoLandingL;
